...
share_updated: 2026-02-01T00:12:13+01:00
...
Started: 2024-10-07 11:20

Status: #devoleped

Tags: [[Programming]], [[Software Engineering]], [[School]], [[3 - Tags/Java|Java]]
...
### To encapsulate
---
Far in modo che i metodi di una classe siano `private`, rendendoli accessibili solo alle classe che li hanno dichiarati (cioè a se stessi).
Fatta eccezione per attributi o metodi che sono `protected`, questi saranno utilizzabili dalle sue sottoclassi.

--- end-multi-column

Le ==interfacce== e ==classi astratte== sono quelle più utilizzate per la programmazione basta sull'astrazione e l'incapsulamento e ne seguono perfettamente le direttive. Con la keyword `interface` o `protocol` vai a creare un _contratto_ tra oggetti. Non troverai mai dentro ad un interfaccia degli attributi, ma solo metodi (behaviors in english).
...
Con ereditarietà si intende come, l'abilità di una classe di poter estendere se stessa, in modo di poter evitare di creare codice duplicato, riutilizzando metodi e i suoi attributi (solo se saranno `protected` o `public`).
...
> [!caution]
> **_Importante:_** una sottoclasse può essere estesa solo da una ed un sola super classe (questo vale in [[6 - The Real Notes/Java/Java|Java]], per altri linguaggi di programmazione non è così).

## 3.4 Polymorphism
![[Screenshot 2026-01-05 at 18.27.32.png|Esempio di Polimorfismo]]
...
>[!example] Esempio 
>Un esempio di composizione è quando si va ad iniettare la classe che si vuole utilizzare all'interno del proprio costruttore, in questo caso:
>```java
...
![[Pasted image 20250925102144.png|23 sono i design pattern che andremmo a vedere]]
...
Se tali interfacce sono gestite correttamente si risolverà il problema, che si può riscontrare solitamente se non si crea a un buon progetto, cioè **minimizzare le dipendenze**. [^3]

>[!question]- **Cosa vuol dire?**
Minimizzare una dipendenza (low cuopling) vuol dire, far in modo che gli oggetti di una classe non si richiamino a vicenda , ma che questo compito venga gestito dall'interfaccia stessa.

Se tali soluzioni vengono applicate per ogni problema, si produrrà del **codice estendibile nel tempo**.

Oltre al codice che dovrà minimizzare le dipendenze tra i vari moduli, si  dovrà creare una classe che avrà una singola responsabilità, cioè far in modo che la classe abbia un'alta coesione, cioè **che sia ben definita la sua singola responsabilità**, cioè capire il ruolo che avrà il modulo all'interno del sistema.

Questo ci permetto di creare un codice mantenibile nel tempo, facile nella comprensione e riutilizzabile in contesti differenti, questo perché non avendo più responsabilità da dover portarsi dietro, questa sua peculiarità gli permette di essere utilizzabili in ambiti differenti.

>[!info]-   **Analogia**
>Ad esempio facciamo conto che un professore insegni una singola materia, cioè **programmazione in Java**, questa sua conoscenza essendo la sua sola ad avere (cioè la sua singola responsabilità), può essere utilizzabile per vari contesti ed ambiti, come ad esempio insegnare alle università e alle scuole superiori, oppure offrire dei corsi a pagamento.
...
Offrono servizi per la __creazione e gestione__ di oggetti in modo più efficiente, invece di utilizzare il new(), forniscono modi intelligenti per farlo.
...
Creano delle separazioni tra interfacce e implementazioni, utili per **modelli complessi** e ci aiutano a comporre classi e oggetti per formare strutture più grandi e flessibili.

--- column-break ---

**Behavioral**
--- 
Modificare il comportamento tra gli oggetti **senza dover modificare grandi porzioni di codice**, cioè definiscono come gli oggetti interagiscono e comunicano tra di loro.
...
Questo principio si avvicina molto all'idea di [[SOLID Principals#I - nterface Segregation| separazione delle interfacce]], cioè che quando andremmo a implementare una classe come ad esempio `Cat`, questo non per forza dovrà dipendere dei comportamenti della classe che andrà a dipendere, in questo esempio `Sausage`.
...
Elenco Dei design pattern creazionali
- [[#6.4.1 Factory pattern|Factory Method]]
- [[#6.4.2 Abstract Pattern|Abstract Method]]
- [[#6.4.3 Builder Pattern|Builder]]
- [[#6.4.4 Prototype pattern|Prototype]]
- [[#6.4.5 Singleton pattern|Singleton]]
...
2. ***Extends Library or Framework***: estendere librerie o framework, come ad esempio [[Programming Knowledge#Overriding (sovrascritto)|sovrascrivere]] un metodo di una classe per fargli fare diverse funzionalità.
...
l'Abstract ci permette di far si che si viene a creare un nuovo mobile in base alla categoria che abbiamo bisogno, in questo caso se il cliente vuole un `Sofa` con la categoria `Victorian`, grazie all'Abstract saremmo in grado di poterlo fare. ^f2083e

>[!tip] Scope
>Lo scopo dell'abstract factory è quello di creare un'interfaccia che va a definire delle famiglie di oggetti che sono correlati o dipendenti, senza conoscerne la sua classe concreta, poi da ogni famiglia generare degli stili differenti.
...
1. **Phase:**
	   Per applicare tale design pattern, ==bisogna prima di tutto creare le interfacce per ogni famiglia di oggetti che vogliamo creare ad esempio ==(sedia, sofa, tavolo) e per ognuna di loro creeiamo la classe che corrisponda alla tipologia/variante dell oggetto (sediaVittoriana, sediaModerna ecc..) ed implementiamo l'interfaccia che abbiamo dichiarato prima.![[Screenshot 2025-09-21 at 15.58.49.png|Prima l'oggetto che vogliamo categorizzare poi la famiglia]]
		Si andrà ad implementare la categoria di famiglia che vogliamo creare, ad esempio se abbiamo `Chair` possiamo poi dopo definire le tipologie di sedie che vogliamo, cioè `Victorian` o `Modern`
> [!info]- **Come si fa in Java?** 
> In Java prima si implementa l'interfaccia della famiglia a cui vogliamo dare delle categorie:
> ```java
...
> }
> ```
> Poi dopo andremmo ad implementare l'interfaccia con le classi che vogliamo siano categorizzate, in questo caso se vogliamo che ci siano 4 categorie: `Victorian`, `Modern`, `Old`, `School` andremmo a creare 4 classi per ogni categoria che definiamo:
> ```java
...
> }
> ```
> questo va fatto per tutte le categorie.
2. **Phase:**
...
> 		protected abstract CoffeTable createCoffeTable();
> }
> ```
> Meglio con il protected, dato che i metodi della classe verranno solo utilizzata dai concreteFactory che estenderanno la classe.
> **N.B**: il tipo di ritorno dei metodi sono le interfacce che abbiamo definito precedentemente, **non sono le singole categorie, ma la famiglia generale**.
3. **Phase:**
	   Si va a creare la **factory concreta** per ogni variante cioè che vuol dire, la parte nella quale si crea ogni classe che genererà una specifica categoria  di mobili, questa classe però dovrà estendere o implementare l'**Abstract Factory**  che abbiamo prima definito.
	   
**Esempio:**
	   `ModernFornitureFactory`  creerà solo `ModernChair`, `ModernSofa` e `ModernCoffeTable` e implementerà l'interfaccia che andrà a creare i singoli oggetti `Chair`, `Sofa`, ecc..
...
3. **Abstract Factory**: si può dichiarare come classe astratta o interfaccia, il factory ha la funzione di dichiarare i metodi di creazione di ciascun abstract product(es. _createChair()_, _createSofa()_ ecc) ==avrà come tipo di ritorno l'interfaccia dei prodotti==.
 ^1163b7
4. **Concrete Factory**: implementerà le dichiarazioni dell'interfaccia **Abstract Factory**, ==queste creerà solo una specifica variante del prodotto==(cioè non può creare un ModernForniture se il suo compito è quello di generare VictorianForniture).
...
Ogni **Concrete Factory** corrisponde ad una variante della famiglia dei prodotti, cioè: **i prodotti di una variante non sono compatibili con prodotti di altri varianti**.

>[!example] Esempio
>Non posso definire una famiglia di componenti UI, con una Variante MacOS che possiede una categoria Windows:
>```java
...
>}
>``` 
>La classe che abbiamo definito deve generare famiglie della categoria `MacOS`, non può generare componenti `Windows`, questo anche se nella firma del metodo possiamo ritornare qualsiasi tipo di bottone o textarea, questo perché sarà da parte nostra fare in modo che si possano definire dei `ConcreteFactory` che generano una singola categoria.
...
> - **Low dependence**: cioè il codice del client non ha una relazione stretta legata ai prodotti concreti.
> - **Single responsibility**: rispetta il principio di [[SOLID Principals#S - ingle responsibility|singola responsabilità]] perché il codice di creazione dei prodotti è centralizzato ad una sola interfaccia che gestisce tale creazione (cioè l'`Abstract Factory`).
...
#### 6.4.2.4 Factory VS Abstract
--- start-multi-column: ID_uk8r
```column-settings
Number of Columns: 2
Largest Column: standard
```
...
- Definisce una sola categoria di prodotti, senza valutare le varianti, in caso si volesse introdurre una variante del prodotto, bisogna violare il principio di [[SOLID Principals#O - pen/ closed|Open Closed]].
- Per creare un nuovo oggetto, bisogna ereditare la classe che si occupa di definire il metodo di creazione di un oggetto e sovrascrivere quel metodo.

**“Modifico quale prodotto creo scrivendo nuove sottoclassi.”**

--- column-break ---

**Abstract**
- Possiamo definire più factory che possiedono varianti differenti, creando diverse famiglie di prodotti.
- Si viene ad usare il concetto della [[#4.3 Composition|composizione]], cioè non c'è la necessità di dover sovrascrivere un metodo per creare una nuova famiglia di prodotti, ma si può semplicemente dare come argomento del costruttore della classe `Client`, cioè  quella che si occupa di generare il tipo di oggetto, la factory che si vuole.
**Abstract Factory → oggetto che crea oggetti → composizione**

--- end-multi-column

#### 6.4.2.5 Relations with the other patterns
- Molti sistemi cominciamo con il design pattern Factory poi si evolvono in [[#6.4.2 Abstract Pattern|Abstract Factory]], [[#6.4.3 Builder Pattern|Builder]] o [[#6.4.4 Prototype pattern|Prototype]] pattern.
- Il  [[#6.4.3 Builder Pattern|Builder]]  si basa sulla creazione di oggetti passo per passo, invece l'Abstract si incentra nella creazione delle famiglie di oggetti con le proprie categorie.
  **Abstract**: ritorna il prodotto/oggetto che si vuole immediatamente appena si va a dichiarare il `ConcreteFactory`
  **Builder:** per poter creare l'oggetto/prodotto che si vuole si devono eseguire degli step prima di creare il prodotto.
- L'[[#6.4.2 Abstract Pattern|Abstract Factory]] può essere utilizzato apposto del [[#7.5 Facade|Facade]], per nascondere come il sistema va a creare gli oggetti al client.
- L'[[#6.4.2 Abstract Pattern|Abstract Factory]] può lavorare bene con il [[#7.2 Bridge Pattern|Bridge]], questa connessione funziona molto bene, l'astrazione e l'implementazione del Bridge non ha delle regole, queste però possono essere definite dall'Abstract, cioè:
  >[!example] Esempio:
  >**Bridge**:  separa _astrazione_ e _implementazione_ (es. _Shape_ e _Color_).
  >A volte, però, **alcune astrazioni possono lavorare solo con certe implementazioni**.
  >Esempio: una classe `VectorShape` deve usare un `VectorRenderer`, non un
  >`RasterRenderer.`
  >
  >**L’Abstract Factory crea la coppia corretta di oggetti per il Bridge.**

 _Il Bridge separa le gerarchie (Remote ↔ Device) ma non garantisce da solo la compatibilità._

 _L’Abstract Factory invece serve proprio a garantire che il Remote corrisponda al tipo di Device._
  

Questa dipendenza logica può essere incapsulata in una **Abstract Factory**, che garantisce che astrazione e implementazione siano sempre _compatibili_.
...
2. **Building step-by-step**:
   Si eseguono dei passi ben precisi per la creazione degli oggetti (`buildDoor`, `buildWall` ecc...) così possiamo costruire solo ciò che ci serve in quel momento, questo tramite il `Director`, che sarebbe un costruttore di oggetti predefinito, noi scegliamo l'oggetto che vogliamo e lui ce lo costruisce seguendo le istruzioni.
...
>[!warning] **N.B**
>Quando si va a creare l'oggetto dal builder, la classe di tale oggetto dovrà avere il costruttore che non sia accessibile, perché così facendo **andremmo a costringere** l'utente a dover utilizzare il builder per poter creare l'oggetto `Car`
![[Screenshot 2025-11-23 at 11.54.26.png]]
#### 6.4.3.3 Builder  when we use it?
- **Evitare la creazione di costruttori con troppi parametri opzionali e complessi** e ci fa in modo di creare l'oggetto, passo dopo passo.
  Si andrà ad evitare di creare **costruttori telescopici**, ma che vuol dire questo?
>[!info] **telescopic constructor**
>Sono una serie di costruttori che vengono richiamati in successione, che servono in modo da far si che si possano creare oggetti che posseggono dei parametri che si vogliono includere oppure non.
>
>Esempio se vogliamo creare delle istanze (oggetti) di `Users`  con solo il nome oppure con `name` e `age`
>```java
>public class User {
...
>}
>```
>Si chiama **telescopic** perché crescono come se fossero un telescopio.

- **Utile  per la creazione di oggetti che hanno la stessa rappresentazione**(es. casa in legno o pietra) utilizzando lo stesso set di passi di costruzione.
...
>[!bug] Problema
>Non possiamo fare la copia di un oggetto di una classe su un'altro oggetto dello stesso tipo, perché alcuni campi possono essere privati o di appartenenza della classe, quindi non possiamo semplicemente assegnare uno ad uno i valori dell'oggetto che vogliamo duplicare.
...
Abbiamo una classe che vogliamo far si che si possa duplicare, per esempio se non sappiamo dell'esistenza del prototype allora faremmo in questo modo:
```java
public class Main{
	public static final main(args[] String){
		Student student = new Student(12345, ..., "Quattrini");
		Studente copyStudent;
		
		copyStudent.setId(student.getId)
		copyStudent.setAge(student.getAge);
		copyStudent.setName(student.getName);
		// ... così via per gl'altri
	}
}
```

Questo processo rimane molto scomodo e non conveniente, per due ragioni principali.

>[!danger] Problems
>1. Andremmo a delegare il compito della duplicazione al Client (classe `Main`) della quale non conoscendo l'oggetto che stiamo clonando(copiando) andremmo a dimenticare degli attributi oppure a rompere il codice in se per sè.
>2. Ci saranno degli attributi `private` che non essendo accessibili a classi esterne, non saremmo in grado di poter svolgere la duplicazione di quest'ultime (anche se avremmo un metodo `getter` che ci fa l'accesso).
...
Stiamo creando l'interfaccia perché così sappiamo che per le classi che vogliono clonare i propri oggetti allora saranno consistenti nei nomi dei metodi che faranno la clonazione
#### 6.4.4.2 Prototype Structure
![[PrototypeStructure.png]]
Vediamo nel dettaglio la struttura del prototype:

1. `«interface»` ***Prototype***: si crea l'interfaccia dove si andrà a creare il metodo che svolgerà la clonazione dove avremmo il singolo metodo `clone()`.
...
## 6.5 Structural Design Patterns
---
...
Qui di seguito avremmo la lista dei pattern strutturali:
- [[#6.5.1 Adapter pattern|Adapter]]
- [[#7.2 Bridge Pattern|Bridge]]
- [[#7.3 Composite Pattern|Composite]]
- [[#7.4 Decorator Pattern|Decorator]]
- [[#7.5 FlyWeight|FlyWeight]]
- [[#7.6 Proxy|Proxy]]
### 6.6 Adapter pattern
Il _design pattern adapter_ ha la funzione di poter prendere in input una specifica interfaccia, incomprensibile per la classe, rendendola accessibile tra di loro.

>[!info] **Analogia**
>Semplicemente l'adapter è come prendere un adattatore per la presa del tipo che ci serve in base al continente che ci troviamo, ad esempio se abbiamo una presa americana, l'adattatore ce la rende del tipo europea e così via per le altre tipologie.
#### 6.6.1  Solution
...
Il compito dell'**Adapter** è quello di nascondere il processo di conversione dal tipo di origine, cioè il file XML della banca, creando un nuovo file in JSON, con i dati  che gli si sono passati.

>[!warning] N.B
>Non vuol dire che l'adapter ha solo il compito di convertire i dati da un formato ad un altro, ma può anche **creare delle connessioni tra le interfacce che sono incompatibili tra loro**
#### 6.6.2  Structure
La struttura del design pattern _adapter_ base è quella che andrà ad utilizzare l'interfaccia della classe che vogliamo convertire e incapsulare o [[#4.4 Aggregation|aggregare]] la classe che verrà convertita.
...
2. Il `«interface» Client interface` conterrà quelle interfacce che che le altre classi dovranno seguire al fine di poter collaborare con la classe `Client`, cioè definisce i vincoli che il client possiede.
...
Capiamo bene il concetto di **astrazione** e **implementazione**:
- **Astrazione**: non svolge logica di business e operazioni importanti, cioè non svolge nessun lavoro vero e proprio.
- **Implementazione**: è quella che andrà a svolgere le operazioni in se per se, cioè che andranno a lavorare con i metodi definiti nell'astrazione.
![[Screenshot 2025-11-25 at 14.25.52.png|Esempio di astrazione e implementazione]]

>[!example] Esempio
>Possiamo prendere come esempio di **astrazione la nostra interfaccia UI** che sviluppiamo per far visualizzare le pagine di un applicazione web.
>
>L'implementazione invece saranno le API che andranno a svolgere le operazioni di CRUD e altro ancora. 
#### 6.7.1 Solution
La soluzione che offre il design pattern bridge è quella di fare dipendere le classi dall'astrazione e non dalle implementazioni concrete, queste attraverso la [[#4.4 Aggregation|composizione]] e non l'ereditarietà.![[Screenshot 2025-11-25 at 14.38.40.png|Esempio di composizione]]
Prendiamo uno dei comportamenti che avevamo inserito tutti all'interno della classe e facciamo in modo che siano separati dalla classe andando a definire un interfaccia, che dopo andrà ad essere inclusa nella classe.

Cosa vuol dire ora?
Quando andremmo a definire un nuovo colore, non andremmo a creare una classe che gestirà il colore, ma basterà solo definire l'interfaccia e l'implementazione dell'interfaccia che ci serve.


Qui sotto abbiamo definito l'*implementazione*, cioè come implementation abbiamo `Color` e le implementazioni concrete sono `Red`, `Blue`.
```java
// Implementor
...
Invece avremmo la struttura dell'`Abstraction` che andrà ad utilizzare l'`implementation`
...
public class Circle extends Shape{
	...
	protected void draw(){
		color.paint();	
		System.out.println("We start to draw a circle, with a radius of " + this.radius)
	}
}
```
Il `Client` non conoscerà l'implementazione dell'interfaccia `Color`, saprà solo che quando utilizzerà l'`Abstraction` cioè la classe `Shape`, sarà in grado di utilizzare un metodo che al suo interno avrà delle funzionalità e in più l'operazione che svolge l'implementazione del colore cioè `paint()`
```java
public class Client{
	public static void main(String[] args){
		...
		circle.draw(); // al suo interno la classe utilizzerà l'implementazione della classe Color.
		
		circle.setColor(blue); // possiamo anche cambiare durante il runtime il tipo di colore senza dove istanziare un'altra figura ed assegnandogli il colore.
	}
}
```

>[!note] N.B
>Quando avremmo l'`abstraction`, non per forza quest'ultima dovrà combaciare con l'interfaccia `implementation`, nel Bridge l'abstraction andrà a **definire la logica di alto livello**, mentre nell'`implementation` si andrà a definire operazioni primitive, cioè di basso livello.
#### 6.7.2 Structure
Questa è la struttura del design pattern Bridge
![[Screenshot 2025-11-25 at 15.02.05.png]]

1. `Abstraction`: definisce la logica di alto livello che dovrà essere poi dopo ridefinita nelle sue sottoclassi, si appoggia sull'implementazione, per svolgere il lavoro duro.
2. `Implementation`: interfaccia che definisce i compiti concreti che andranno a svolgere le sue sottoclassi, cioè va a definire come le `Concrete Implementation` andranno a definire i metodi.
3. `Concrete Implementation`: contiene il codice che viene implementato.
4. `Refined Abstraction`(optional): possono implementare gli stessi metodi che sono stati definiti dalla classe `Abstraction`.
5. `Client`: il client andrà ad utilizzare l'`Abstraction` che dovrà poi istanziare un oggetto dell'implementazione dentro all'`Abstraction`

#### 6.7.3 When We use it?
>[!bug]-  Quando si vuole dividere ed organizzare una classe monolitica (cioè una classe che possiede molte funzionalità, tutte in un unico progetto).
>Il Bridge ci permette di prendere una classe che possiede molte funzionalità e fare in modo che si possano dividere in diverse gerarchie. Questo perché se dividess

>[!bug]- Utilizzi il pattern quando vuoi fare in modo di creare delle gerarchie di classi indipendenti. 
> Cioè quando si vuole fare dei cambiamenti su una classe, quest'ultimi non andranno da influire altre classi, ma saranno indipendenti da tutti.

>[!bug] Quando vuoi fare lo switch tra diverse implementazioni durante runtime.

Vediamo un esempio di quest'ultimo.

Definiamo un interfaccia `Color`
```java
interface Color{
	void paint();
}
...
Le *Concrete Implementation* che saranno `Blue` e `Red`:
```java
public class Red implements Color{
	public void paint(){
...
Definiamo poi l'*Abstraction* che sarà colui che definirà l'*implementation* non sapendo quale *Concrete Implementation* verrà definito dal `Client`.
...
public class Shape{
	...
	class void setColor(Color color){
		this.color = color;
	}
...
public class Circle extends Shape{
	...
	class Circle(Color color,  double radius){
		super(color);
	}
...
```
In questo caso abbiamo definito una *Refined Abstraction*, cioè stiamo dicendo che il Cerchio andrà ad implementare il metodo `paint` che è presente nell'*Implementation*, non sapendo però quale utilizzeremmo.
...
public class Client{
	public static void main(String[] args) {
		...
		Color color4 = new Brow();
		
		Shape circle = new Circle(color2);
		// Domanda: che istanza di Color stiamo utilizzando?
		// Risposta: ... stiamo utilizzando l'istanza di ()
		
		/** 
		Hint: capelli di una ragazza con cui mi sentivo.
		Non è difficile la risposta (se mai leggerai questa parte,
		congratulazioni che sei arrivata fino a questo punto della
		tua carriera, sapevo che saresti riuscita nei tuoi
		obbiettivi, ti voglio bene <3, buona fortuna con l'esame).
		**/
		
		circle.paint();
		// Qui ci si può confondere con lo Strategy perché sembra che stiamo cambiando algoritmo durante il runtime, però la differenza è che non conosciamo l'implementazione del colore, invece nello stategy sappiamo l'implementazione che utilizziamo.
		circle.setColor()
	}
}
```

>[!question] Che differenza c'è tra **Bridge** e **Strategy**?
>**Bridge:** pennarello magico, in base a dove colori il colore (implementation) cambia, ma chi colora non sa che colore andrà ad utilizzare.
**Strategy:** pennarello con colore scelto, se scelgo il pennarello rosso (strategy rosso), so che andrò a colorare in rosso
### 6.8 Composite Pattern
---
*Composite* ci permette di gestire gli oggetti di una famiglia che sono aggregati tra solo, questo con l'utilizzo di un interfaccia comune ed applicabile su tutta la gerarchia.

Prendiamo l'esempio del libro, si deve gestire degli ordini che sono presenti all'interno di una scatola `Box`, questa scatola però ci possono essere o dei singoli prodotti, `Product` oppure delle altre `Box`, in questo caso però se ci sono altre `Box` si dovrà nuovamente aprire quest'ultime e poi calcolare il prezzo dei singoli prodotti.
![[Screenshot 2026-01-06 at 10.34.39.png| La struttura per intero è un albero al roverscio]]

Questa operazione, rimane semplice da un punto di vista 'umano', cioè nella realtà si può fare, ma per un programma rimane molto più complicato di quello che sembra.

>[!warning] **N.B**
>Questo design pattern ha senso utilizzarlo solamente quando dovremmo gestire strutture dati che sono rappresentabili gerarchicamente (alberi o grafi mi viene pensato come esempio).

#### 6.8.1 Solution
La soluzione che offre il *Composite* è quella di definire un interfaccia comune a i contenitori di o oggetti (nel nostro esempio precedente le `Box`) sia per gli oggetti che sono all'interno dei contenitori (`Product`), che dichiareranno un metodo che accumenerà entrambi le classi.

Il funzionamento è semplice:
- Sei un `Product` allora il metodo che ti sei implementato dall'interfaccia ti fai ritornare il prezzo
- Il contenitore  (cioè la `Box`), il metodo che  implementerà non ti ritornerà il prezzo, ma scorre al suo interno finché non trova il prodotto e si ritorna il prezzo.

Questo metodo ti permette di evitare di far dipendere il `Client` dalla classe concreta, ma dalla sua interfaccia, poi sarà il contenitore a svolgere le operazioni più complesse.
#### 6.8.2 Structure
![[Screenshot 2026-01-07 at 15.33.11.png| Struttura del composite]]

1. **`Interface` Component**: Interfaccia comune sia alla `Leaf` che al `Composite`, cioè definisce un operazione comune da dare al componente figlio (quello che non ha più operazioni aggiuntigve) che il contenitore (andrà a delegare l'operazione ai suoi figli).
   
2. **Leaf**: Classe che implementerà l'interfaccia, svolgendo le operazioni più complesse, dato che non ha più nessuno a cui delegare il lavoro, sarà quello che farà il lavoro duro.
   
3. **Composite**: Classe contenitore che potrà contenere sia delle foglie `Leaf` che altri contenitori, questo la classe `Composite` non lo sa, sa solo che i suoi figli saranno del tipo `Component` (questo attraverso il [[#3.4 Polymorphism|polimorfismo]], se non ve lo ricordate vedetevelo di nuovo)

4. **Client**: Lavorerà con i figli  di`Component`,  senza preoccuparsi di chi andrà a fare cosa per primo, il compito viene delegato al `Leaf` e al `Composite`.
#### 6.8.3 When we use it?
>[!bug]- Quando stiamo trattando con gerarchie ad albero.
>Questo pattern ci è utile quando vogliamo far in modo di creare una gerarchia di oggetti ben dettagliata. Questo perché possiamo avere sia elementi figli `Leaf` che i  contenitori `Composite`, che possono contente altri contenitori o elementi figli.

>[!bug]- Quando vogliamo che il `Client`, gestisca gli oggetti complessi e semplici alla stessa maniera.
>Oggetti complessi sono i contenitori, quelli semplici sono le foglie, usiamo questo pattern quando non vogliamo che ci sia una distinzione fra i due, ma li mettiamo allo stesso livello.

#### 6.8.4 Pros and Cons
>[!done] **Pros**
>- Possiamo utilizzare a pieno il concetto del *polimorfismo*, per poter navigare nella struttura ad albero.
>  
>- Seguiamo il principio SOLID [[SOLID Principals#O - pen/ closed|Open-Closed]], perché se volessimo introdurre un nuovo elemento `Leaf` non andremmo a rompere il codice, ma ci basterà farlo implementare all'interfaccia `Component` o estenderlo ad un elemento foglia.

>[!fail] **Cons**
>- In alcuni scenari è complicato delegare la stessa interfaccia ad elementi che svolgono operazioni completamente diverse dal loro compito principale.

>[!warning] Attenzione Potresti rischiare di violare il principio SOLID [[SOLID Principals#I - nterface Segregation| interface segregation]].
> Se andassi a mettere i metodi `add` e `remove` nell'interfaccia `Component`, violerai il principio sopra citato, questo perché quando andrai a realizzare la `Leaf` avrai.
> ```java
> public class Leaf{
> 	public void execute(){
> 		// Qualeche operazione.
> 	}
> 	public void add(){} //rimarranno vuoti perché non farai nessuna operazione di aggiunta o rimozione
> 	public void remove(){}
> }
> ``` 
...
---
>[!info] 
>E' un pattern Strutturale che permette di inserire nuovi comportamenti agli oggetti, inserendo gli oggetti che vogliamo aggiungere dei nuovi comportamenti a dei contenitori (*wrapper* chiamati nel libro).

Prendiamo l'esempio di un sistema di messaggi email, chiamato `Notifier`, questo sistema avrà un costruttore:

`Notifier notifier = new Notifier("common message")` 

questa che definirà il messaggio che dovremmo mandare. La classe avrà un metodo per esempio che invia i messaggi ad una serie di email che sono state definite al suo interno: `notifier.doSomething()`.
![[Screenshot 2026-01-10 at 16.24.23.png|Esempio di invio notifica da un sistema esterno]]

Nello sviluppo del sistema però si voglio aggiungere altri metodi diversi di invio delle notifiche, per esempio tramite SMS, Facebook, Instagram oppure Slack, se facessimo tramite [[#3.3 Inheritance (ereditarietà)|ereditarietà]], si possono incontrare diverse problematiche, tra cui:

1. **Non si può cambiare lo stato di un oggetto che esiste in runtime**
2. **Il cambiamento della classe padre andrebbe a violare il Principio SOLID** [[SOLID Principals#O - pen/ closed|Open-Closed]], rompendo l'intero codice.
3. **Molti linguaggi di programmazione supportando l'ereditarietà singola**, cioè una sotto-classe non può essere estesa da più di una  super-classe (esempio semplice un figlio *sotto-classe* non può avere padri differenti *super-classi*), limitando la possibilità di un figlio di avere metodi/comportamenti, da diversi padri.

Quindi si opta ad utilizzare il concetto della [[#4.3 Composition|composizione]], molto importante 
![[Screenshot 2026-01-10 at 16.51.19.png| Ereditarietà vs. Composizione]]
##### 6.9.1 Solution
La soluzione sarebbe quella di racchiudere le classi in questo *Wrapper*, cioè un contenitori nella quale ci sono dei *target*, cioè delle classi che erediteranno Il comportamento del *Wrapper*, ma avranno delle funzionalità aggiuntive che il padre (cioè il *Wrapper*) non ha, per esempio:
![[Screenshot 2026-01-11 at 13.12.05.png]]
Avremmo il nostro `Component` che è il `Notifier`, che verrà realizzato dalla classe `BaseDecorator`, ma che questo metodo non solo sarà delegato al `BaseDecorator`, ma anche ai propri figli, che oltre che sono obbligati ad utilizzare il metodo del padre (questo tramite la `super.send(message)`) avranno un metodo al loro interno che avrà una funzionalità in più `sendSMS(message)`.
##### 6.9.2 Structure
![[Screenshot 2026-01-10 at 16.59.42.png]]
1. `Component` : definisce l'interfaccia comune per gli oggetti che sono contenitori e componenti (`Concrete Component` e `Base Decorator`).
   
2. `Concrete Component`: definisce la classe degli oggetti che verranno messi all'interno del contenitore, è l'**oggetto di base da cui partiamo**.
   
3. `Base Decorator`: fa la dependency injection del `Component`, facendo riferimento nel metodo `execute()` a quello che c'è all'intero della `Concrete Component`, rendendo disponibile l'implementazione base ai `Concrete Decorators`, cioè gli oggetti che sono contenuti.

4. `Concrete Decorators`: definiscono dei comportamenti aggiuntivi a quelli che sono definiti nel `Base Decorator`, questo tramite la dichiarazione di un metodo `extra()`, che verrà eseguito dentro all'`execute()` insieme a quello del padre.
```java
public class ConcreteDecorators extends BaseDecorator{
	private Component c;
	class ConcreteDecorators(Component c){
		this.c = c;
	}
	
	@Override
	public void execute(){
		super.execute(); // Esempio farà la print di "Hello!"
		extra(); // Qui si aggiungerà la print di "Extra operation!"
	}
...
```
5. Il `Client`: andrà a prendere il `Concrete Component`, che sarebbe l'oggetto base che vogliamo aggiungere delle funzionalità, e con i `ConcreteDecorator` gli vai ad aggiungere delle componenti.
```java
Component baseComponent = new ConcreteComponent("danilo.quattrini");

// execute(){ super.execute; extra(); }
// extra(){ System.out.println("@gmail"); }
BaseDecorator domain = new GmailDomain(baseComponent);

// execute(){ super.execute; extra(); }
// extra(){ System.out.println(".com"); }
BaseDecorator nation = new NationDomain(domain):

nation.execute()
// ConcreteComponent -> Decorator -> Decorator
// danilo.quattrini + @gmail + .com = danilo.quattrini@gmail.com
```

##### 6.9.3 When we use it?

>[!bug] Quando vogliamo aggiungere dei comportamenti  degli oggetti durante il runtime senza rompere il codice.

>[!bug]- Quando non è possibile estendere o aggiungere un nuovo comportamento alla classe, tramite l'utilizzo dell'ereditarietà.
>In Java ad esempio, l'operatore `final` non permette di cambiare lo stato di una variabile o di estendere la classe e fare delle modifiche su quest'ultime. L'unico modo che c'è per poter modificare tale variabile sarebbe quello di usare il Decorator come design patter.

##### 6.9.4 Pros and Cons

>[!done] Pro
>- **Estendere il comportamento di una classe senza dover creare una nuova sotto classe**
>- **Aggiungere o rimuovere responsabilità da un'istanza di una classe durante il runtime.**
>- Combinare diversi comportamenti, raggruppando l'oggetto dentro in diversi decorator.
>- R**ispetta il principio di Single Resnponsibility perché da una classe monolitica, si riesce a dividerla in classi piccole che implementano varianti più dettagliate di un comportamento.**

>[!fail]  Cons
>- Difficoltà di rimuovere un Decorator, da un' insieme di Decorator inzializzati uno dentro l'altro.
>- E' difficile implementare un Decorator in cui il comportamento non dipenda dall'ordine dell'implementazione.
>- Lo stile del codice, dove inizializzi il decorator rimane brutto esteticamente.
### 7.0 Facade
---
>[!info] **Definizione**
>E' un patter Strutturale che ti permette di interagire un interfaccia, una libreria, un framework o un insieme di classi complesse,  attraverso una singola e semplice interfaccia.

Non modifica il comportamento interno del sistema, ma **nasconde la complessità** dietro a un punto di accesso più facile da usare. 

#### 7.0.1 Solution

Questo ad esempio può essere utile se invece di dove conoscere l'intero framework o libreria che dobbiamo utilizzare, andremmo solo ad interagire con un singolo metodo, che andrà ad include le principali funzionalità del framework o della libreria, che abbiamo bisogno.

>[!example] **Esempio**
>Se dovessimo chiamare un negozio, per dover svolgere un ordine su un prodotto o un servizio, **L'operatore telefonico sarà il Facade che interagirà a posto tuo con tutti i servizi e componenti del negozio**.
>![[Screenshot 2026-01-10 at 18.35.28.png]]

#### 7.0.2 Structure

![[Screenshot 2026-01-10 at 18.36.07.png]]
1. `Facade`: classe che conosce il sottosistema, sa dove dover indirizzare il `Client` e come muovere le principali parti del sistema.
   
2. `Additional Facade`: serve per far si che il `Facade`, non diventi complesso quanto il `Complex Subsystem`, definendo un metodo che potrà essere utilizzato dal `Client` e dal `Facade`.
   
3. `Complex System`: il sottosistema che svolge le operazioni complesse, queste non conoscono minimamente l'esistenza della classe `Facade`, ma implementano la vera logica del sistema 

4. `Client`: utilizza il `Facade` per svolgere le operazioni, senza dover interagire direttamente con il `Complex System`.

#### 7.0.3 When we use it?

>[!bug] **Quando si vuole avere un unica interfaccia, che svolgerà le operazioni complesse di un sistema** 

>[!bug]- **Quando vuoi definire la struttura del `Complex System` in layer** 
>Cioè quando si definisce un Facade si va a definire una struttura a livelli, dove ad esempio possiamo dividere il sistema in due parti, definendo due metodi che sono diversi tra di loro, ma che possono comunicare ed inviarsi informazioni a vicenda.

#### 7.0.4 Pros and Cons

>[!done] **Pros**
>- Isolare il codice, dalla complessità di un sottosistema (libreria framework e sistemi vari).

>[!fail] **Cons**
>- Il `Facade` può diventare una **God Class** (cioè una classe con molte responsabilità) se non si gestisce bene quello che andrà a fare.

### 7.1 FlyWeight
---
>[!warning]- **N.B**
>Questo pattern è molto difficile quindi consiglio di vederlo molto lentamente, si capisce il suo scopo, ma deve essere visto passo per passo.

>[!info] **Definizione**
>**FlyWeight** è un pattern Strutturale che viene utilizzato per ridurre la quantità di memoria (RAM) che occupa una classe. Questo separando delle componenti ( o chiamate variabili), tra diversi oggetti differenti, che però si comunicano a vicenda.

Concetto fondamentale da conoscere per il FlyWeight è la differenza che si trova tra i dati *Intrinsechi* ed *estrinsechi*:
- **Intrinseco**: un dato intrinseco è quel dato che si mantiene nel tempo e rimane sempre lo stesso durante il suo ciclo di vita
  >[!example] **Esempio**
  >Prendiamo per esempio del proiettile del libro, la `sprite` e il `color` sono delle variabili che non cambieranno il loro stato durante il ciclo di vita dell'oggetto `Particle` :![[Screenshot 2026-01-12 at 15.16.53.png]]
  >sono valori che non cambiano durante il corso di vita dell'oggetto
  
  - **Estrinseco**: qualsiasi dato che cambia il suo stato durate il ciclo di vita, ad esempio, prendiamo come dato le variabili `x`, `y`, `speed`.

>[!warning] **IMPORTANTE**
>Quello che il pattern cerca di risolvere è **far in modo di isolare le variabili intrinseche da quelle estrinseche**. Nell'esempio qui sotto, vediamo come viene suddivise le variabili sopra citate.
![[Screenshot 2026-01-12 at 15.23.28.png]]

#### 7.1.1 Structure
![[Screenshot 2026-01-12 at 15.31.51.png]]

>[!warning] **Quando Usarlo?**
>Il pattern viene utilizzato principalmente quando si ha un consumo di RAM eccessivo, se non c'è altra soluzione, lo applichiamo al nostro codice, quindi **lo usiamo solo quando consumiamo tanta memoria**.

1. `Flyweight`: classe che contiene come variabili i dati che non cambiano durante il corso di vita di un oggetto (cioè quelli intrinsechi), il metodo `operation(uniquieState)`, prende come input le variabili che cambiano (cioè quelle estrinseche).
   
	*N.B* : anche il costruttore del `FlyWeight` prende le variabili estrinseche, creando l'oggetto in se per se.

2. `Context`: classe che contiene le variabili estrinseche, cioè quelle che sono differenti per ogni oggetto che viene creato. Questa **classe è fondamentale**, perché quando si unisce con uno delle classi `FlyWeight`, rappresenta l'intero stato dell'oggetto (prima solo con il `FlyWeight` definivamo solo gli stati che non cambiano, con il `Context` collegato, saremmo in grado di rappresentare l'oggetto intero).
3. Il `Client` aggiunge o calcola i valori estrinsechi da aggiungere al `FlyWeight`, per il client il `FlyWeight` sarebbe come se fosse un oggetto di default che viene preso per far in modo di poter modificare il proprio stato a piacimento. 

4. `FlyWeight Factory` : quella classe che mantiene una Lista o Array di `FlyWeight`, cioè il `Client` passa i valori intrinsechi al `FlyWeight Factory`, quest'ultimo controlla nell'array di `Factory` se i parametri inseriti siano già esistenti in uno dei suoi elementi. Se esista allora richiama il `FlyWeight` esistente, caso contrario ne crea uno nuovo.

>[!danger] Nota Importante.
>1. **Il `Flyweight `NON conosce il `Context`**
>2. **Il `Context` conosce il `Flyweight`**

#### 7.1.2 When We use it?
>[!bug]- Lo usiamo quando dobbiamo ridurre l'utilizzo della quantità di memoria RAM, facendo in modo di far entrare tanti oggetti in uno spazio limitato.
> Esempio prendiamo una scatola e delle palline da ping-pong,  se sappiamo che la scatola può contenere solo 100 palline, non andremmo ad aggiungerne altre 100, se no si riempe la scatola.
> - Applicazione che utilizza tanta memoria RAM, con questo design pattern la riduciamo.
> - Classi che devono inizializzare oggetti che sono identici.

#### 7.1.3 Pros vs Cons
>[!done] Pro
>**Salviamo tanta quantità di memoria RAM per far si che si possa far girare oggetti che hanno la stessa struttura**

>[!fail] Cons
>- Salviamo tanta memoria RAM ma andremmo ad utilizzare tanta CPU, questo perché quando si va a chiamare il context questo va a ricalcolare e a richiamare di nuovo il flyweight.
>- Codice complicate e il tuo team si chiederà perché hai diviso i concetti in questo modo.

### 7.2 Proxy
---
Design Pattern Strutturale che si comporta come da filtro o bamboccio per l'oggetto originale, questo per far si che le chiamate che vengono svolte dall'esterno, sarranno gestite in modo gerarchico
Cosa intendo con questo? In base alla disponibilità dell'oggetto che conosce il *Proxy*, questo potrà essere acceduto oppure no.

>[!info] Definizione
>Il design pattern Proxy controlla gli accessi agli oggetti che si vogliono chiamare dall'esterno.
## 8.0 Behavioral Design Patterns
### 8.x State vs Strategy
>[!danger] Che differenza c'è tra State e Strategy? 
>**State**: Il design pattern ha la caratteristica di conoscere gli oggetti che vengono realizzati dall'interfaccia `<<interface>>State` e riesce e transitare tra uno stato ad un'altro, conosce anche il **Context**.
>**Strategy**: non conosce le realizzazione dell'interfaccia Strategy e non conosce l'esistenza del **Context**
...
[^3]: Spiegazione del *Coupling* tra i vari moduli [Accoppiamento](https://www.perplexity.ai/search/dependency-inversion-explanati-b60nk3hSQpSPU89MSxLqRw#2)